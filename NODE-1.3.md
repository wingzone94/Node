...
**ローカルで続けるための移植**: 上記の検証ページは `scratch/`（gitignore対象）にしか無く
リポジトリには載らない。ローカルには本物の WordPress があるので、同じ判定を
**実サイトに対して**回せる `scripts/mobile-check.mjs` を追加した（`bun run verify:mobile`）。
判定内容は 390px で LATEST 3件・境界700/701・タップ領域44px・カテゴリ名の省略・
`node-flow` スクリプトの不在・横スクロール無し・フッター列数・検索サジェストの位置と省略。
未捕捉例外だけを失敗にし、画像などの読み込み失敗は INFO に落としてある
（テーマ外の1件でチェック全体が落ちないようにするため）。
