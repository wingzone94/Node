...
* **HEADLINE の表示切替をピルボタンに**（外枠 999px の受け皿 + 選択中は塗りの丸）。実測で `views` / `view` とも `border-radius: 999px`
...
### 追記（同日）: 動画を確認し、回遊性の欠落へ追加対応

Drive・YouTube はどちらも組織のエグレスポリシーで遮断されていたが、ユーザーが
圧縮した .mov をセッションへ直接アップロードしてくれたため、ffmpeg でフレームを
切り出して**動画の中身を実際に確認できた**（35.76秒 / 296×640 / iPhone実機）。

**動画で確定した事実**:

* 指を止めずに約30秒スクロールし、**8/19 → 5/28 の約3か月ぶんの記事をほぼ全部通過**
* 32秒地点でフッターに到達するが、**「すべての記事を見る」が存在しない**
  （スクローラーが隠し、全ページ読み切ってローダーも消えるため何も残らない）

当初の実装（無限スクロール廃止・モバイル3件）は症状と一致していたが、
**それだけでは行ける先が減るだけ**だと判明。サイト内にカテゴリ一覧が一箇所も無く、
カード上のバッジを踏む以外にカテゴリアーカイブへ行く手段が無かった。

**追加した回遊導線**（詳細は NODE-1.3.md §25.6）:

* `node_get_navigation_categories()`（記事数順 / hide_empty / 未分類除外）
* `template-parts/category-nav.php` — トップの `#categories` セクション。**LATEST の上**に配置
* フッターにカテゴリ列（`.m3-footer__grid` を 2列 → 3列）
* モバイル追従ナビに `CATEGORY` を追加（既存のスクロールスパイがそのまま拾う）
