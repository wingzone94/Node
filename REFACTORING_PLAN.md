...
> **現行ステータス（2026-08-01）: 旧計画として凍結。** 本書は `codex/release-1.1.3` の作業前スナップショットと「R-0から1項目1コミット」を前提とするため、Node 1.3の大量WIPが存在する現在の `master` へ直接適用しない。安全な考え方は現行の段階的リファクタリングへ引き継ぐが、R-0〜R-11を未処理の現行タスクとして数えない。再開する場合はNode 1.3リリース後に新しい棚卸しから計画を作り直す。
