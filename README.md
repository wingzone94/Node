...
![Version](https://img.shields.io/badge/version-0.6.2-orange?style=for-the-badge)
...
### 追加・変更点
- **読了プログレスバーの高度化**: 
    - 冗長だったサークル型ゲージを廃止し、バー型に一本化。
    - スクロールに応じた伸縮（可逆性）に対応し、記事の最後まで到達した際にバーが弾ける「粉砕アニメーション」を実装。
- **SNSシェアアイコンの刷新**: 
    - Bluesky アイコンを初期の「雲」デザインに回帰させ、テーマのクラシックな質感と調和。
    - Misskey アイコンを公式準拠のサークルMスタイルに更新し、ブランド認知を向上。
- **RSSアイコンの公式配色採用**: 
    - Firefox等で標準的な RSS オレンジ (#F26522) を採用。クリック・ホバー時のインタラクションを維持。
- **検索UIのシンボル更新**: 
    - 詳細検索のトリガーおよびモーダル内アイコンを「ろうと (filter_alt)」に変更。
    - ゲームプラットフォーム選択を最大5つまでに制限し、超過時には「検索を実行」ボタンが震える警告アニメーションを導入。
- **Material 3 Expressive ローディング**: 
    - 検索実行時の待機画面に、流動的なモーフィングとバブルエフェクトを組み合わせた M3E 仕様のローディングアニメーションを導入。
